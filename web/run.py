...
def connectdb():
	db = pymysql.connect(host='localhost', user='root', passwd='110120cary', db='cwh', port=3306, charset='utf8')
...
def rate():
	(db,cursor) = connectdb()
	cursor.execute("select rate, district, category, showtime, mvlength from movie")
	movies = cursor.fetchall()
	temp = []
	for item in movies:
		if not item['showtime'] == 0 and not item['mvlength'] == 0:
			temp.append(item)
	movies = temp
